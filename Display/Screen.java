...
public class Screen extends JPanel
  ...
  static {
    if (DebugMode) System.out.println("Welcome to Debug Mode!");
    // setups logger
      ...
      case SHEET_EDIT -> {
        ...
        try {
          switch (menuState) {
            ...
            case EMIT_SELECT -> {
              if (!emitPanel.isVisible()) emitPanel.setVisible(true);
            }
            case ADD_CUT -> {
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == addSheet) {
      ...
      File temp =
          new File(selectedSheet.getParentFile().getPath() + "\\" + newCutField.getText() + ".cut");
      ...
      gcodePartPanel =
          new PartSelectGcode(((SheetHandlerJButtonCut) e.getSource()).getgenericThing());
      add(gcodePartPanel);
      ...
      for (JButton button : suffixes) {
        if (e.getSource() == button) {
          ...
          if (finalOrder != null) {
            File outputFolder =
                new File("./output/" + java.time.LocalDate.now().toString().replaceAll("-", ""));
            outputFolder.mkdir();
  ...
  /**
   * Allows instanceof to be detected for returning to home in actionPerformed() and implements
  ...
  /**
   * Allows instanceof to be detected for returning once(with the returnable interface) in
   * actionPerformed() and implements cloneable to allow the same text and location for subsequent
   * buttons
   *
   * @see JButton
  ...
  /** The emit menu that allows which extension(aka endmill size) to emit and the new file name */
  ...
  private static class ToolOrderDialog extends JDialog {
    ...
    public ToolOrderDialog(java.awt.Frame owner, List<Integer> tools) {
      ...
      for (Integer t : tools) listModel.addElement(t);
      ...
      public boolean importData(TransferSupport support) {
        try {
          ...
          if (fromIndex < 0
              ...
              || toIndex < 0
              || toIndex > listModel.getSize()) return false;
